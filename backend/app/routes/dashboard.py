...
from app.services import dashboard_service, recurring_service, subscription_service
...
@dashboard_bp.route("/api/dashboard/subscriptions", methods=["GET"])
@login_required
def subscriptions():
    return jsonify(subscription_service.detect_subscriptions(g.current_user["id"])), 200
