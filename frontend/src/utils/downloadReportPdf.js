...
function statBlock(doc, x, y, width, label, value, color) {
...
export function downloadReportPdf({ period, userName, transactions, range }) {
  ...
  statBlock(doc, 14, cardY, cardWidth, "Total Income", formatMoney(income), INCOME);
  statBlock(doc, 14 + cardWidth + gap, cardY, cardWidth, "Total Expenses", formatMoney(expenses), EXPENSE);
  statBlock(
    doc,
    14 + (cardWidth + gap) * 2,
    cardY,
  ...
  if (topCategories.length > 0) {
    ...
    doc.setFont(undefined, "bold");
    doc.text("Top Spending Categories", 14, cursorY);
    doc.setFont(undefined, "normal");
  ...
  doc.setTextColor(...INK);
  doc.setFont(undefined, "bold");
  doc.text("Transaction Detail", 14, cursorY);
  doc.setFont(undefined, "normal");
